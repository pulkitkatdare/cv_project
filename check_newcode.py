			...
			if (dot1 > 0):
				...
				if (dot2 > 0 ): 
					...
					if(dot3 > 0 ):
						...
						if (((float(k_test1)+float(k_test2)+float(k_test3))/(float(count1+0.00001)+float(count2+0.00001)+float(count3+0.00001))) > 0.8):
							#print 1
							if ((vote[i-1,3]+vote[j-1,3]+vote[k-1,3]) > maxval):							
								maxval = vote[i-1,3]+vote[j-1,3]+vote[k-1,3] 
								print maxval
								i_arg = i-1
						 		j_arg = j-1
								k_arg = k-1
